...
class DataConnection(object):
    ...
    def _data_item_url(self, resource_name, resource_id):
        ...
        """
        return "{}/data_api/v1/{}/{}/?format=json".format(self.configurator.get_config()['server'],
    ...
    def register(self, registration_token=None, file_name=None):
        ...
        response = requests.post(self._api_url('local_computer'),
                                 data=json.dumps(self.configurator.get_config()),
    ...
    def create_event(self, event_type, info):
        ...
        response = self.client.post(url, data=json.dumps({'type': event_type, 'info': info,
                                                          'local_computer_id': config['id']}),
                                    headers=self.post_header())
    ...
    def sec_header(self, base_header=None):
        auth_header = {'auth_key': self.configurator.get_config()["secret_uuid"],
    ...
    def post_header(self):
        auth_header = {'auth_key': self.configurator.get_config()["secret_uuid"],
                       'content-type': 'application/json', "X-CSRFToken": self.csrf}
        return auth_header
    ...
    def check_response_ok(cls, response):
        ...
        if 200 <= response.status_code < 300:
            ...
            response_string = "WARNING Command lookup failed: status: {} reason: {}".format(response.status_code,
                                                                                            response.reason)
            if response.content:
