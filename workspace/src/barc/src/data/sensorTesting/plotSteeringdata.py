...
file_data = open(sys.path[0]+'/Steering_oldBARC.obj', 'rb')
...
steer_raw = pickle.load(file_data) 
...
plt.figure(2)
plt.plot(time, steer_raw, '-ob', label="steer_raw")
plt.plot(time, yaw_raw, '-or', label="roll_raw")
plt.plot(time, [1*a for a in pitch_raw], '-og', label="pitch_raw")
plt.legend()
