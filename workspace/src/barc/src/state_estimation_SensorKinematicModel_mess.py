...
def state_estimation():
    ...
    while not rospy.is_shutdown():
        ...
        y = array([se.x_meas, se.y_meas, se.vel_meas,  se.psiDot_meas, se.a_x_meas, se.a_y_meas, 0*sin(bta)*se.vel_meas])
        ...
        l.Localize(x_est,   y_est,   psi_est)
        ...
        # state_pub_pos.publish(pos_info(Header(stamp=ros_t), l.s, l.ey, l.epsi, v_x_est, se.x_est, se.y_est, v_x_est, v_y_est,
        #                                psi_est, psi_dot_est, se.a_x_meas, se.a_y_meas, se.cmd_motor, se.cmd_servo))
    ...
    np.savez(pathSave,yaw_est_his       = psi_est_his,
                      psiDot_est_his    = psi_dot_est_his,
                      x_est_his         = x_est_his,
                      y_est_his         = y_est_his,
    ...
    np.savez(pathSave,psiDot_his    = se.psidot_raw_his,
                      roll_his      = se.roll_raw_his,
                      pitch_his     = se.pitch_raw_his,
                      yaw_his       = se.yaw_raw_his,
                      ax_his        = se.a_x_meas_his,
                      ay_his        = se.a_y_meas_his,
                      imu_time      = se.imu_time)

    pathSave = os.path.join(homedir,"barc_debugging/estimator_gps_mess.npz")
    np.savez(pathSave,x_his         = se.gps_x_his,
                      y_his         = se.gps_y_his,
                      gps_time      = se.gps_imu_fusion_time)

    pathSave = os.path.join(homedir,"barc_debugging/estimator_enc_mess.npz")
    np.savez(pathSave,v_fl_his          = [v/2 for v in vel_meas_his],
                      v_fr_his          = [v/2 for v in vel_meas_his],
                      v_rl_his          = [v/2 for v in vel_meas_his],
                      v_rr_his          = [v/2 for v in vel_meas_his],
