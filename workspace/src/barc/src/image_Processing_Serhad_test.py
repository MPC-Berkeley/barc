...
#	gray_image = cv2.bilateralFilter(gray_image, 11, 15, 11)
	gray_image = gray_image[int(0.8*rows):rows, int(0.065*cols):cols]
...
#			cv2.waitKey(1)
	...
	#	filtered = signal.fftconvolve(self.kernel, gray_image)
	#	threshold = np.percentile(filtered, self.quartile)
	#	thresh = ipm.copy()

	#	blur = cv2.medianBlur(gray_image, int(5))
	        blur = cv2.bilateralFilter(gray_image, 3, 255, 255)
	#	blur = gray_image
	#	blur = cv2.bitwise_not(blur)
		image_bw = cv2.threshold(blur, 135, 255, cv2.THRESH_BINARY)[1]
		image_bw[0:, 0] = 255
		image_bw[0:, 299] = 255
		
		
    
#		image_bw = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 11, 17)

#		image_bw[image_bw < 90] = 255

#		filtered[filtered < threshold] = 0
#		filtered[filtered >= threshold] = 255
#		filtered = np.array(thresh, dtype = np.uint8)
		filtered = image_bw.copy()
#		(frow, fcols, fchannels )= filtered.shape()	
...
#	
	## Lane Detection
		...
		if display_image or publish_image:
			cv2.circle(backtorgb, (x_left, height - index_y), 3, (0, 128, 255), -1)
			cv2.circle(backtorgb, (x_right, height - index_y), 3, (0, 128, 255), -1)
