...
function SE_callback(msg::pos_info,lapStatus::LapStatus,posInfo::PosInfo,mpcSol::MpcSol,a_est::Array{Float64,1},z_est::Array{Float64,1},x_est::Array{Float64,1})
    z_est[:]= [msg.v_x,msg.v_y,msg.psiDot,msg.epsi,msg.ey,msg.s] # the last variable is filtered acceleration
...
function main()
    ...
    const FEATURE_FLAG     = true  # true:8-shape,     false:history (this requires the corresponding change in 3 palces)
    ...
    z_est            = zeros(6)          # (xDot, yDot, psiDot, ePsi, eY, s, acc_f)
    a_est            = zeros(2)
    ...
    s1          = Subscriber("pos_info", pos_info, SE_callback, (lapStatus,posInfo,mpcSol,a_est,z_est,x_est),queue_size=1)::RobotOS.Subscriber{barc.msg.pos_info}
    ...
    log_cvy             = zeros(BUFFERSIZE,mpcParams.N,1,num_lap)
    log_cpsi            = zeros(BUFFERSIZE,mpcParams.N,1,num_lap)
    solHistory          = SolHistory(BUFFERSIZE,mpcParams.N,8,num_lap)
    ...
    mdl_convhull_simple  = MpcModel_convhull_dyn_iden_simple(mpcParams,modelParams,selectedStates)
    ...
    if !PF_FLAG
        ...
        (~,~,~)=solveMpcProblem_convhull_dyn_iden_simple(mdl_convhull_simple,mpcSol,mpcCoeff,rand(6),rand(mpcParams.N+1,6),rand(mpcParams.N,2),selectedStates_dummy,track,rand(mpcParams.N),rand(mpcParams.N))
        (~,~,~)=car_pre_dyn(rand(1,6)+1,rand(mpcParams.N,2),track,modelParams,6)
        (~,~,~)=find_SS_dist_simple(solHistory,rand(1,6),rand(1,2),lapStatus,selectedStates)
    end
    ...
    while ! is_shutdown()
        if z_est[6] > 0    
            ...
            if lapStatus.nextLap
                ...
                setvalue(mdl_convhull_simple.z_Ol[1:mpcParams.N,1],mpcSol.z[2:mpcParams.N+1,1]-posInfo.s_target)
                setvalue(mdl_convhull_simple.z_Ol[mpcParams.N+1,1],mpcSol.z[mpcParams.N+1,1]-posInfo.s_target)
                ...
                if lapStatus.currentLap > num_lap # to save the data at the end before error pops up
                    ...
                    save(log_path,"log_cvx",log_cvx,"log_cvy",log_cvy,"log_cpsi",log_cpsi,"GP_vy_History",GP_vy_History,"GP_psidot_History",GP_psidot_History,
				                  "selectedStates",selectedStates,"oldSS",oldSS,"solHistory",solHistory,
            ...
            if lapStatus.currentLap<=1+max(selectedStates.feature_Nl,selectedStates.Nl) # pF CONTROLLER
                ...
                if LMPC_FLAG
                    ...
                    if !TI_TV_FLAG
                        ...
                        for i in 1:mpcParams.N
                            if FEATURE_FLAG
                                ...
                                (iden_z,iden_u,z_iden_plot)=find_feature_dist_simple(feature_z,feature_u,z_to_iden[i,:],u_to_iden[i,:],selectedStates)
                            else
                                if NORM_SPACE_FLAG
                                    # SELECTING THE FEATURE POINTS FROM HISTORY BASED ON 2-NORM CRITERION
                                    (iden_z,iden_u,z_iden_plot)=find_SS_dist_simple(solHistory,z_to_iden[i,:],u_to_iden[i,:],lapStatus,selectedStates)
                                else
                            ...
                            (mpcCoeff.c_Vx[i,:],mpcCoeff.c_Vy[i])=coeff_iden_dist_simple(iden_z,iden_u)
                        end
                        println(iden_z)
                        println(iden_u)
                        mpcCoeff.c_Psi = z_to_iden[:,6]
                    else
                        # TI SYS_ID
                        if FEATURE_FLAG
                            # SELECTING THE FEATURE POINTS FROM DATASET
                            ...
                            if NORM_SPACE_FLAG
                                ...
                                (iden_z,iden_u,z_iden_plot)=find_SS_dist_simple(solHistory,z_curr',u_prev[2,:],lapStatus,selectedStates)
                            else
                                (iden_z,iden_u,z_iden_plot)=find_feature_space(solHistory,z_curr',u_prev[2,:],lapStatus,selectedStates,posInfo)
                            end
                        end
                        (mpcCoeff.c_Vx[1,:],mpcCoeff.c_Vy[1,:],mpcCoeff.c_Psi[1,:])=coeff_iden_dist_simple(iden_z,iden_u)
                    ...
                    tic()
                    (mpcSol.z,mpcSol.u,sol_status)=solveMpcProblem_convhull_dyn_iden_simple(mdl_convhull_simple,mpcSol,mpcCoeff,z_curr,z_prev,u_prev,selectedStates,track,GP_e_vy,GP_e_psidot)
                    toc() # TIME FOR OPTIMIZATION
                end

                # COLLECT ONE STEP PREDICTION ERROR FOR GPR 
                if !GP_LOCAL_FLAG && !GP_FULL_FLAG && lapStatus.currentIt>1
                    feature_GP_z[k,:]       = solHistory.z[lapStatus.currentIt-1,lapStatus.currentLap,1,1:6]
            ...
            if PF_FLAG || (!PF_FLAG && lapStatus.currentLap > 1+max(selectedStates.feature_Nl,selectedStates.Nl))
                ...
                solHistory.z[lapStatus.currentIt,lapStatus.currentLap,1,4:6]=z_curr[4:6]
                solHistory.z[lapStatus.currentIt,lapStatus.currentLap,1,7:8]=a_curr
