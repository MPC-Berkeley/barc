...
def offset_callback(data):
	global pixel_offset, pixel_offset_prev # This stores the current and previous values of lane offset in pixels 
	if data is not None:
...
L = 0.261
l = 0.3566
steeringGain = 2.54/ 1600 # inch/pixels

def steering_command(rateHz, Kp, Kd, Ki):
	global pixel_offset, pixel_offset_prev, ei
	e = pixel_offset # current error
	ed = e - pixel_offset_prev # derivative of error
	ei = ei + ( e / float(rateHz)) # integral of error
	
	turn = (e * Kp) + (ed * Kd) + (ei + Ki) #Calculates turn angle
