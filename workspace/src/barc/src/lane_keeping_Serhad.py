...
def steering_command(rateHz, Kp, Kd, Ki):
	...
	d_f = turn
	e_prev = e
	return (angle_2_servo(-turn) - 2), -turn, e	
...
def speed_command(speed_desired, rateHz, KpS, KdS, KiS, offset):
...
def main_auto():
	...
	# get steering Controller Parameters P I D
	...
	#open csv file
	test1 = open('/home/odroid/barc/workspace/src/barc/src/test4.csv', 'w')
	
	# here starts the main loop
	while not rospy.is_shutdown():
		
			
		servoCMD, angle, offset = steering_command(rateHz, Kp, Kd, Ki)
		speedCMD = speed_command(speed_desired, rateHz, KpS, KdS, KiS, offset) # Get steering command signal
		...
		save_data = (speedCMD, servoCMD, offset)
		result = "%f,%f,%f" % save_data + "\n"
		test1.write(result)
		# Wait
		rate.sleep()
