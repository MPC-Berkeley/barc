...
# Output is a difference value indicating the difference (offset) of the vehicle from the center of the lane
	...
	gray_image = gray_image[int(0.7*rows):, 0:cols]
	...
	# here we apply filtering to the image using kernel
	        blur = cv2.bilateralFilter(gray_image, 5, 150, 150)
		image_bw = cv2.threshold(blur, 165, 255, cv2.THRESH_BINARY)[1]
		filtered = image_bw.copy()
	
			
#	# Here we convert back to RGB to visualize the lane detection points
		if display_image:
			cv2.imshow('Filtered Image', image_bw)
			cv2.waitKey(1)
