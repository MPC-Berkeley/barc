...
using Ipopt
using Types
export MdlPf,MdlKin,MdlId,MdlIdLin,MdlDynLin,MdlDynLin3,MdlKinLin
    type MdlPf
    ...
    type MdlKin
        ...
        function MdlKin(agent::Agent)
            ...
            u_ub = [ 1.5     18/180*pi]
            z_lb = [-Inf -Inf -Inf -0.5] # 1.s 2.ey 3.epsi 4.v
            z_ub = [ Inf  Inf  Inf  Inf] # 1.s 2.ey 3.epsi 4.v
    ...
    type MdlId
        ...
        function MdlId(agent::Agent)
            ...
            z_ub    = [ Inf  Inf  Inf  Inf  Inf  Inf] # 1.s 2.ey 3.epsi 4.vx 5.vy 6.psi_dot
    ...
    type MdlIdLin
        ...
        function MdlIdLin(agent::Agent)
            ...
            # 
            @variable( mdl, z_Ol[1:(N+1),1:6], start=0.1)
            @variable( mdl, u_Ol[1:N,1:2], start=0.01)
            @variable( mdl, eps_lane[1:N] >= 0)   # eps for soft lane constraints
            @variable( mdl, alpha[1:Nl*Np] >= 0)    # coefficients of the convex hull
            ...
            @NLparameter(mdl, GP_psiDot_e[i=1:N] == 0)
            @NLparameter(mdl, c[1:N] == 0)
            @NLparameter(mdl, c_Vx[1:N,1:4]  == 0)
            @NLparameter(mdl, c_Vy[1:N,1:4]  == 0)
            ...
            for i=1:N
                if i == 1
                    ...
                    @NLconstraint(mdl, z_Ol[i+1,2]  == z_Ol[i,2] + agent.mpcParams.dt * (z_Ol[i,4]*sin(z_Ol[i,3]) + z_Ol[i,5]*cos(z_Ol[i,3])))  # eY
                    @NLconstraint(mdl, z_Ol[i+1,3]  == z_Ol[i,3] + agent.mpcParams.dt * (z_Ol[i,6]-dsdt[i]*c[i]))   # ePsi
                    @NLconstraint(mdl, z_Ol[i+1,4]  == z_Ol[i,4] + (agent.mpcParams.dt/dt)*c_Vx[i,1]*z_Ol[i,4] + 
                                                                   (agent.mpcParams.dt/dt)*c_Vx[i,2]*z_Ol[i,5] +
                                                                   (agent.mpcParams.dt/dt)*c_Vx[i,3]*z_Ol[i,6] +
                                                                   (agent.mpcParams.dt/dt)*c_Vx[i,4]*u_Ol[i,1] +
                                                                   GP_vx_e[i])
                                                                   # (agent.mpcParams.dt/dt)*c_Vx[i,5]*u_Ol[i,2] +
                    @NLconstraint(mdl, z_Ol[i+1,5]  == z_Ol[i,5] + (agent.mpcParams.dt/dt)*c_Vy[i,1]*z_Ol[i,4] + 
                                                                   (agent.mpcParams.dt/dt)*c_Vy[i,2]*z_Ol[i,5] + 
                                                                   (agent.mpcParams.dt/dt)*c_Vy[i,3]*z_Ol[i,6] + 
                                                                   (agent.mpcParams.dt/dt)*c_Vy[i,4]*u_Ol[i,2] + 
                                                                   GP_vy_e[i])                      # vy
                    @NLconstraint(mdl, z_Ol[i+1,6]  == z_Ol[i,6] + (agent.mpcParams.dt/dt)*c_Psi[i,1]*z_Ol[i,4] + 
                                                                   (agent.mpcParams.dt/dt)*c_Psi[i,2]*z_Ol[i,5] +
                                                                   (agent.mpcParams.dt/dt)*c_Psi[i,3]*z_Ol[i,6] + 
                                                                   (agent.mpcParams.dt/dt)*c_Psi[i,4]*u_Ol[i,2] + 
                                                                   GP_psiDot_e[i])                  # psiDot
                else
                    @NLconstraint(mdl, z_Ol[i+1,1]  == z_Ol[i,1] + dt * dsdt[i])                    # s
                    @NLconstraint(mdl, z_Ol[i+1,2]  == z_Ol[i,2] + dt * (z_Ol[i,4]*sin(z_Ol[i,3]) + z_Ol[i,5]*cos(z_Ol[i,3])))  # eY
            ...
            @NLexpression(mdl, derivCost, sum{QderivZ[j]*sum{(z_Ol[i,j]-z_Ol[i+1,j])^2,i=1:N},j=1:6} +
                                              ...
                                              QderivU[1]*(uPrev[delay_a+1,1] -u_Ol[delay_a+1, 1])^2 + 
                                              QderivU[2]*(uPrev[delay_df+1,2]-u_Ol[delay_df+1,2])^2 )
            @NLexpression(mdl, laneCost,    Q_lane*sum{1.0*eps_lane[i]+40.0*eps_lane[i]^2 , i=1:N})
            @NLexpression(mdl, terminalCost,Q_term_cost*sum{alpha[i]*stateCost[i] , i=1:Nl*Np})
            @NLexpression(mdl, slackCost,   sum{Q_slack[i]*(z_Ol[N+1,i]-sum{alpha[j]*selStates[j,i],j=1:Nl*Np})^2, i=1:6})
    ...
    type MdlKinLin
        ...
        function MdlKinLin(agent::Agent)
            ...
            @variable( mdl, z_Ol[1:N,1:4], start=0.1)
            @variable( mdl, u_Ol[1:N,1:2], start=0.1)
            @variable( mdl, eps_lane[1:N] >= 0)   # eps for soft lane constraints
            @variable( mdl, alpha[1:Nl*Np] >= 0)  # coefficients of the convex hull
    ...
    type MdlDynLin3
        ...
        function MdlDynLin3(agent::Agent)
            m = new()
            L_a     = get_param("L_a")
            L_b     = get_param("L_b")
            # dt      = get_param("controller/dt")
            ...
            @NLconstraint(mdl, [i=1:N], z_Ol[i,2] <= ey_max + eps_lane[i])
            @NLconstraint(mdl, [i=1:N], z_Ol[i,2] >= -ey_max - eps_lane[i])
            @constraint(mdl, sum{alpha[i] , i=1:Nl*Np} == 1)

            @NLconstraint(mdl, [i=1:N], z_Ol[i,4] <= z_ub[4])
            @NLconstraint(mdl, [i=1:N], z_Ol[i,4] >= z_lb[4])
            for j=1:2
                @NLconstraint(mdl, [i=1:N], u_Ol[i,j] + u_linear[i,j] <= u_ub[j])
                @NLconstraint(mdl, [i=1:N], u_Ol[i,j] + u_linear[i,j] >= u_lb[j])
            end

            @NLexpression(mdl, bta[i=1:N],  L_a/(L_a + L_b)*(u_Ol[i,2] + u_linear[i,2]))
            @NLexpression(mdl, dsdt[i=1:N], (z_Ol[i,4]+z_linear[i+1,4])*cos(z_Ol[i,3]+bta[i])/(1-z_Ol[i,2]*c[i]))
            
            # System dynamics
            for i=1:N-1
                for j=1:6
                    if j==4
                        @NLconstraint(mdl, z_Ol[i+1,j] == sum{A[j,k,i+1]*z_Ol[i,k],   k=4:6} + 
                                                          sum{B[j,k,i+1]*u_Ol[i+1,k], k=1:2})# +
                                                          # GP_vx_e[i+1] )
                    elseif j==5
                        @NLconstraint(mdl, z_Ol[i+1,j] == sum{A[j,k,i+1]*z_Ol[i,k],   k=4:6} + 
                    ...
                    elseif j==6
                        @NLconstraint(mdl, z_Ol[i+1,j] == sum{A[j,k,i+1]*z_Ol[i,k],   k=4:6} + 
                                                          ...
                                                          # GP_psiDot_e[i+1] )
                    elseif j==1
                        @NLconstraint(mdl, z_Ol[i+1,1] == z_Ol[i,1] + dt*dsdt[i+1])                                                   # s
                    elseif j==2    
                        @NLconstraint(mdl, z_Ol[i+1,2] == z_Ol[i,2] + dt*(z_Ol[i,4]+z_linear[i+1,4])*sin(z_Ol[i,3]+bta[i+1])     )     # ey
                    elseif j==3    
                        @NLconstraint(mdl, z_Ol[i+1,3] == z_Ol[i,3] + dt*((z_Ol[i,4]+z_linear[i+1,4])/L_b*sin(bta[i+1])-dsdt[i+1]*c[i+1]) )   # epsi
                    end
                end
            end
            for j=1:6
                ...
                elseif j==6
                    @NLconstraint(mdl, z_Ol[1,j] == sum{B[j,k,1]*u_Ol[1,k], k=1:2} )
                elseif j==1
                    @NLconstraint(mdl, z_Ol[1,j] == z0[j]+ dt*dsdt[1])
                elseif j==2
                    @NLconstraint(mdl, z_Ol[1,j] == z0[j]+ dt*z0[4]*sin(z0[3]+bta[1]))
                elseif j==3
                    @NLconstraint(mdl, z_Ol[1,j] == z0[j]+ dt*(z0[4]/L_b*sin(bta[1])-dsdt[1]*c[1]))
                end
            end
            ...
            # end
            @NLexpression(mdl, derivCost, sum{QderivZ[j]*(sum{(z_Ol[i,j]+z_linear[i+1,j]-z_Ol[i+1,j]-z_linear[i+2,j])^2,i=1:N-1}+
                                                          (z_Ol[1,j]+z_linear[2,j]-z_linear[1,j])^2), j=4:6} +
                                              QderivU[1]*sum{(u_Ol[i,1]+u_linear[i,1]-u_Ol[i+1,1]-u_linear[i+1,1])^2,i=delay_a+1:N-1} + 
                                              QderivU[2]*sum{(u_Ol[i,2]+u_linear[i,1]-u_Ol[i+1,2]-u_linear[i+1,1])^2,i=delay_df+1:N-1} + 
                                              QderivU[1]*(uPrev[delay_a+1,1] -u_Ol[delay_a+1, 1]-u_linear[delay_a+1, 1])^2 + 
                                              QderivU[2]*(uPrev[delay_df+1,2]-u_Ol[delay_df+1,2]-u_linear[delay_df+1,1])^2 )
            ...
            @NLexpression(mdl, laneCost,    Q_lane*sum{10.0*eps_lane[i]+50.0*eps_lane[i]^2, i=1:N})
            @NLexpression(mdl, terminalCost,Q_term_cost*sum{alpha[i]*stateCost[i] , i=1:Nl*Np})
            @NLexpression(mdl, slackCost1,   sum{Q_slack[i]*(z_Ol[N,i]+z_linear[N+1,i]-sum{alpha[j]*selStates[j,i],j=1:Nl*Np})^2, i=4:6})
            @NLexpression(mdl, slackCost2,   sum{Q_slack[i]*(z_Ol[N,i]                -sum{alpha[j]*selStates[j,i],j=1:Nl*Np})^2, i=1:3})

            @NLobjective(mdl, Min, derivCost + laneCost +  terminalCost + slackCost1 + slackCost2)
...
import CarSim:carPreDyn, carPreId
    ...
    function solveDynLin3(mdl::MdlDynLin3,agent::Agent)
        z_curr = [agent.posInfo.s,agent.posInfo.ey,agent.posInfo.epsi,agent.posInfo.vx,agent.posInfo.vy,agent.posInfo.psiDot]
        ...
        setvalue(mdl.z0,      z_curr[1:4])
        setvalue(mdl.c,       curvature[1:end-1])
        setvalue(mdl.z_linear,z_linear)
        setvalue(mdl.u_linear,u_linear)
        ...
        agent.mpcSol.sol_status  = solve(mdl.mdl)
        agent.mpcSol.u = getvalue(mdl.u_Ol) + u_linear
        agent.mpcSol.z = vcat(z_linear[1,:], hcat(getvalue(mdl.z_Ol[:,1:3]),getvalue(mdl.z_Ol[:,4:6]) + z_linear[2:end,4:6]))
