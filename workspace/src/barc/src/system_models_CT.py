...
def h_KinBkMdl_withGPS(x,u,vhMdl,dt):
    """
    measurement model
    """
    psi = x[2]
    ...
    x_meas = x[0]-(L_b+0.070)*cos(psi)+0.033*sin(psi)
    y_meas = x[1]-(L_b+0.070)*sin(psi)-0.033*cos(psi)
    psi_meas = x[2]
    v_meas = x[3]

    return array([x_meas, y_meas, psi_meas, v_meas])
