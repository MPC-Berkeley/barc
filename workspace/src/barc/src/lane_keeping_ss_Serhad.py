...
def offset_callback(data):
	global pixel_offset # This stores the current and previous values of lane offset in pixels 
	if data is not None:
...
def steering_command(rateHz):
	global pixel_offset, L, l, steeringGain
	steeringAngle = 0
	
	if (pixel_offset > 0):
		e = pixel_offset * steeringGain
		steeringAngle = L/sqrt(sin(atan2(e,1))/1/e) # convert to degrees 
	elif (pixel_offset < 0):
		e = -pixel_offset * steeringGain
		steeringAngle = -L/sqrt(sin(atan2(e,1))/1/e)
...
def main_auto():
	...
	while not rospy.is_shutdown():
		...
		servoCMD, angle = steering_command(rateHz) # Get steering command signal
