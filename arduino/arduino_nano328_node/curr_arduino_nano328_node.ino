...
float Car::getVelocityEstimate() {
    ...
    if (FL_DeltaTime > 0) {
        vel_FL = (2.0 * 3.141592654 * 0.036 / 8.0) * (1.0/FL_DeltaTime*1000000.0);
    }

    if (FR_DeltaTime > 0) {
        vel_FR = (2.0 * 3.141592654 * 0.036 / 8.0) * (1.0/FR_DeltaTime*1000000.0);
    }

    if (BL_DeltaTime > 0) {
        vel_BL = (2.0 * 3.141592654 * 0.036 / 8.0) * (1.0/BL_DeltaTime*1000000.0);
    }

    if (BR_DeltaTime > 0) {
        vel_BR = (2.0 * 3.141592654 * 0.036 / 8.0) * (1.0/BR_DeltaTime*1000000.0);
    }
